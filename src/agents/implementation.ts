...
export class ImplementationAgent extends BaseAgent<ImplementationResult> {
  ...
  private implPath(): string {
    return `src/feature-${this.ctx.issue.number}.js`;
  }
  private testPath(): string {
    return `tests/feature-${this.ctx.issue.number}.test.js`;
  }
  private slug(): string {
  ...
  private renderImpl(state: AgentState): string {
    ...
    return [
      ...
      ``,
      `export function feature${this.ctx.issue.number}(input) {`,
  ...
  private async writeTest(): Promise<void> {
    const testBody = [
      ...
      `import { feature${this.ctx.issue.number} } from "../src/feature-${this.ctx.issue.number}.js";`,
      ``,
      `test("feature${this.ctx.issue.number} returns success for valid input", () => {`,
      `  assert.deepEqual(feature${this.ctx.issue.number}({ ok: true }), { state: "success", message: "done" });`,
      `});`,
      ``,
    ...
    const testPath = `tests/feature-${this.ctx.issue.number}.test.js`;
    const abs = path.join(this.ctx.repo.workdir, testPath);
